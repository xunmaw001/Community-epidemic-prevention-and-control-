<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'" label="代购编号" prop="daigoubianhao">
            <el-input v-model="ruleForm.daigoubianhao" 
                placeholder="代购编号" readonly></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.daigoubianhao" label="代购编号" prop="daigoubianhao">
              <el-input v-model="ruleForm.daigoubianhao" 
                placeholder="代购编号" readonly></el-input>
          </el-form-item>
        </div>
...
export default {
  ...
  methods: {
	...
	this.ruleForm.shangpinxiangqing = this.ruleForm.shangpinxiangqing.replace(reg,'../../../ssmh39cl/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `daigouwupin/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.daigouwupinCrossAddOrUpdateFlag = false;
						 this.parent.search();
