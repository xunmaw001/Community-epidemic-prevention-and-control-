<template>
    ...
    <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			<picker v-if="queryList.length>1" mode="selector" :range="queryList" range-key="queryName" :value="queryIndex" @change="queryChange" style="padding-left: 20upx;">
				...
				<input v-model="searchForm.xiaodumingcheng" type="text" placeholder="消毒名称" ></input>
			</view>
                                                                                          			<view v-if="queryIndex==1" class="search-form round">
				<text class="cuIcon-search"></text>
				<input v-model="searchForm.xiaoduriqi" type="text" placeholder="消毒日期" ></input>
			</view>
        ...
        <view @tap="onDetailTap(product)" class="uni-product" :style='{"padding":"0px 20rpx 0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0 1% 20rpx 2%","borderColor":"rgba(130, 163, 157, 1)","backgroundColor":"#fff","borderRadius":"20rpx","borderWidth":"2rpx","width":"47%","borderStyle":"solid","height":"auto"}' v-for="(product,index) in list" :key="index">
                                <view class="uni-product-title" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"#333","textAlign":"center","borderRadius":"0","borderWidth":"0","width":"100%","fontSize":"28rpx","lineHeight":"56rpx","borderStyle":"solid"}'>{{product.xiaodumingcheng}}</view>
            ...
            <text v-if="userid && isAuth('xiaodujilu','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="!userid && isAuthFront('xiaodujilu','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="userid && isAuth('xiaodujilu','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
            <text v-if="!userid && isAuthFront('xiaodujilu','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
          </view>
        </view>
      </view>
            <!-- 样式2 -->
	...
	export default {
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.xiaodumingcheng="";
				this.searchForm.xiaoduriqi="";
			},
			//类别搜索
        ...
        params['sort'] = 'xiaoduriqi';
        params['order'] = 'desc';


				if(this.searchForm.xiaodumingcheng){
					params['xiaodumingcheng'] = '%' + this.searchForm.xiaodumingcheng + '%'
				}
				if(this.searchForm.xiaoduriqi){
					params['xiaoduriqi'] = '%' + this.searchForm.xiaoduriqi + '%'
				}
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`xiaodujilu`, params);
                } else {
                    res = await this.$api.list(`xiaodujilu`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('xiaodujilu', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				if(this.searchForm.xiaodumingcheng){
					searchForm['xiaodumingcheng'] = '%' + this.searchForm.xiaodumingcheng + '%'
				}
				if(this.searchForm.xiaoduriqi){
					searchForm['xiaoduriqi'] = '%' + this.searchForm.xiaoduriqi + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`xiaodujilu`, searchForm);
                } else {
                    res = await this.$api.list(`xiaodujilu`, searchForm);
                }
				// 如果是第一页数据置空
